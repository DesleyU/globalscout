...
using GlobalScout.Infrastructure.PlayerIdentity;
...
public static class DependencyInjection
{
        ...
        IConfiguration configuration)
    {
        ...
        services.AddHttpClient<IExternalPlayerSearch, ApiFootballPlayerSearch>((sp, client) =>
        {
            ConfigureApiFootballHttpClient(sp, client);
        });
    ...
    private static void ConfigureApiFootballHttpClient(IServiceProvider sp, HttpClient client)
    {
        var o = sp.GetRequiredService<IOptions<ApiFootballOptions>>().Value;
        client.BaseAddress = new Uri(o.BaseUrl.TrimEnd('/') + "/");
        if (!string.IsNullOrWhiteSpace(o.ApiKey))
        {
            client.DefaultRequestHeaders.TryAddWithoutValidation("x-apisports-key", o.ApiKey);
        }
