...
public sealed class AccountIntegrationTests
{
    ...
    [Fact]
    public async Task Post_set_role_returns_401_without_token()
    {
        using var client = _fixture.Factory.CreateClient();
        using var response = await client.PostAsJsonAsync(
            "/api/account/role",
            new { role = AppRoleNames.Player },
            Ct);
        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task Post_set_role_pending_user_becomes_player()
    {
        var (_, pendingToken) = await SocialIntegrationTestHelpers.RegisterUserAsync(_fixture.Factory, Ct);
        using var client = SocialIntegrationTestHelpers.CreateAuthenticatedClient(_fixture.Factory, pendingToken);
        ...
        var doc = await JsonDocument.ParseAsync(stream, cancellationToken: Ct);
        var root = doc.RootElement;
        Assert.Equal(AppRoleNames.Player, root.GetProperty("user").GetProperty("role").GetString());
        Assert.False(string.IsNullOrWhiteSpace(root.GetProperty("token").GetString()));
    }

    [Fact]
    public async Task Post_set_role_rejects_admin()
    {
        var (_, pendingToken) = await SocialIntegrationTestHelpers.RegisterUserAsync(_fixture.Factory, Ct);
        using var client = SocialIntegrationTestHelpers.CreateAuthenticatedClient(_fixture.Factory, pendingToken);
        ...
        using var response = await client.PostAsJsonAsync(
            "/api/account/role",
            new { role = AppRoleNames.Admin },
            Ct);
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
    }

    [Fact]
    public async Task Post_set_role_when_already_set_returns_400()
    {
        (_, string playerToken) = await SocialIntegrationTestHelpers.RegisterPlayerUserAsync(_fixture.Factory, Ct);
        using var client = SocialIntegrationTestHelpers.CreateAuthenticatedClient(_fixture.Factory, playerToken);

        using var response = await client.PostAsJsonAsync(
            "/api/account/role",
            new { role = AppRoleNames.Club },
            Ct);
        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
    }
}
