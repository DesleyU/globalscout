...
public sealed class SocialConnectionsIntegrationTests
{
    ...
    public async Task SendConnection_Success_Returns201_WithConnectionPayload()
    {
        ...
        var (senderId, senderToken) = await SocialIntegrationTestHelpers.RegisterClubUserAsync(factory, Ct);
    ...
    public async Task SendConnection_ReverseExists_Returns400()
    {
        var factory = _fixture.Factory;
        var anon = factory.CreateClient();
        var (aId, aToken) = await SocialIntegrationTestHelpers.RegisterClubUserAsync(factory, Ct);
        var (bId, bToken) = await SocialIntegrationTestHelpers.RegisterClubUserAsync(factory, Ct);
    ...
    public async Task Respond_NotReceiver_Returns404()
    {
        ...
        var (_, senderToken) = await SocialIntegrationTestHelpers.RegisterClubUserAsync(factory, Ct);
        var (receiverId, _) = await SocialIntegrationTestHelpers.RegisterClubUserAsync(factory, Ct);
        var (_, otherToken) = await SocialIntegrationTestHelpers.RegisterClubUserAsync(factory, Ct);
    ...
    public async Task ConnectionLimit_BasicUser_Returns403_On11thOutgoingRequest()
    {
        ...
        for (var i = 0; i < 12; i++)
        {
            users.Add(await SocialIntegrationTestHelpers.RegisterClubUserAsync(factory, Ct));
        }
